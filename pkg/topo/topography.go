...
func (t *Topography) Height(x, y fixed.F16) fixed.F32 {
	...
	for depth := 0; depth < t.Depth; depth++ {
		...
		height += t.Noise.V((xt.Times(t.Rotations[depth].Cos) + yt.Times(t.Rotations[depth].Sin)).F16(), (yt.Times(t.Rotations[depth].Cos) - xt.Times(t.Rotations[depth].Sin)).F16()).F16().Times(t.Scales[depth].Amplitude)
	}
